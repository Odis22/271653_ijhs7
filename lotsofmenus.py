...
catalogItem2 = CatalogItem(user_id=1, name="Summerslam 2016", description="Brock Lesnar vs Randy Orton",
                     price="$5.50", item="DVDS", restaurant=restaurant1)

session.add(catalogItem2)
session.commit()

catalogItem3 = CatalogItem(user_id=1, name="WWE 2k17", description="for PS4 and Xbox one",
                     price="$3.99", item="videogames", restaurant=restaurant1)

session.add(catalogItem3)
session.commit()

catalogItem4 = MenuItem(user_id=1, name="Royal Rumble", description="who will go to the main event of wrestlemania",
                     price="$7.99", item="DVDS", store=store1)

session.add(menuItem4)
session.commit()

catalogItem5 = MenuItem(user_id=1, name="Monday night Raw live", description="March 15 at united center in chicago illinois at 7:00pm",
                     price="$1.99", item="Tickets", store=store1)

session.add(menuItem5)
session.commit()

catalogItem6 = MenuItem(user_id=1, name="WWE smackdown", description="March 16 at all state center in chicago illinois at 7:00pm"",
                     price="$.99", item="Tickets", store=store1)

session.add(menuItem6)
session.commit()

catalogItem7 = MenuItem(user_id=1, name="The Best of NXT",
                     description="Best NXT matches", price="$3.49", item="DVDS", store=store1)

session.add(menuItem7)
session.commit()

catalogItem8 = MenuItem(user_id=1, name="NXT Takeover:Respect", description="Becky Lynch vs Bayley",
...
catalogItem4 = MenuItem(user_id=1, name="Final Deletion", description="Matt hardy vs Jeff Hardy",
                     price="$7.99", item="DVDS", store=store1)

session.add(menuItem4)
session.commit()

catalogItem5 = MenuItem(user_id=1, name="TNA impact", description="tickets to the live events",
                     price="$1.99", item="Tickets", store=store1)
...
catalogItem5 = MenuItem(user_id=3, name="ROH live events", description="tickets to live shows and payperviews.",
                     price="$1.99", item="Tickets", store=store1)
